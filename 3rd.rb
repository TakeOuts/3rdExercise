...
loop do
  ...
  @api_paste_name = gets.chomp
  
  puts "Введите содержимое документа: "
  @api_paste_code = gets.chomp
  
  # Если введенный текст не является пустой строкой и не превышает лимиты, то выходим из цикла
  unless @api_paste_name.empty? || @api_paste_code.empty?
    if @api_paste_name.length < 100 && @api_paste_code.length < 500
      break
    else
      puts "Превышен лимит длины названия или документа"
    end    
